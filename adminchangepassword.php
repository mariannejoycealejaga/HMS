...
{
	// $sql = "SELECT * FROM admin WHERE loginid='$_POST[adminid]'";
	// $qsql = mysqli_query($con,$sql);
	// $res = mysqli_num_rows($qsql);
	// if($res == 1)
	// {
	// 	$row=mysqli_fetch_assoc($qsql);
	// 	if (password_verify($_POST[oldpassword], $row['password'])){
	// 		$sql2 = "UPDATE admin SET password='$_POST[newpassword]' WHERE adminid='$_SESSION[adminid]'";
	...
	$sql = "UPDATE admin SET password='$hashed_password' WHERE adminid='$_SESSION[adminid]'";
	$qsql= mysqli_query($con,$sql);
