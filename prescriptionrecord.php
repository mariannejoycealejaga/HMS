...
if(isset($_POST[submit]))
{
	...
	{
			$sql ="UPDATE prescription_records SET prescription_id='$_POST[prescriptionid]',medicine_name='$_POST[medicine]',cost='$_POST[cost]',unit='$_POST[unit]',dosage='$_POST[select2]', notes= '$_POST[textarea]',status=' $_POST[select]' WHERE prescription_record_id='$_GET[editid]'";
	...
	{
		$sql ="INSERT INTO prescription_records(prescription_id,medicine_name,cost,unit,dosage, notes,status) values('$_POST[prescriptionid]','$_POST[medicineid]','$_POST[cost]','$_POST[unit]','$_POST[select2]', '$_POST[textarea]','Active')";
		if($qsql = mysqli_query($con,$sql))
		{	
...
<form method="post" action="" name="frmpresrecord" onSubmit="return validateform()"> 
  <input type="hidden" name="prescriptionid" value="<?php echo $_GET[prescriptionid]; ?>"  />
    <table class="table table-striped">
      <tbody>
        ...
        <tr>
          ...
          <?php
		  $arr = array("0-0-1 (One pill in the evening)","0-1-1 (Take a pill in the afternoon and evening)","1-0-1 (Twice a day,  morning and evening)","1-1-1 (Take that pill thrice a day)","1-1-0 (Take a pill, Morning and Afternoon)","0-1-0 (Take pill once in afternoon only)","1-0-0 (Once a day in the morning)");
		  foreach($arr as $val)
		  {
		...
		<td>Note</td>
          <td><input class="form-control" name="textarea" id="textarea" cols="45" rows="5"><?php echo $rsedit[note] ; ?></textarea></td>
        </tr>
        <tr>
    ...
    <a href="doctor_print_prescription.php?prescriptionid=<?= $_GET[prescriptionid]; ?>" target="_blank" class="btn btn-default">Print Prescription</a>
